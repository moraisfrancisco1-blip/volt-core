...
def _seconds_until_sweep_due() -> float:
    ...
    # an extra paid LLM call just because the process happened to restart. A failed
    # report does not count as covering the interval, so a genuine failure gets retried
    # on the next restart rather than waiting out a full week.
    with session_scope() as session:
        latest = session.scalar(
            select(DaiOakesIntelligenceReportRecord)
            .where(DaiOakesIntelligenceReportRecord.status == "completed")
            .order_by(DaiOakesIntelligenceReportRecord.id.desc())
        )
