...
from .escalations import dispatch_phone_call, queue_escalation, router as escalation_router, sync_escalation_status
...
def create_event(session, payload: EventIngestion, principal: Principal) -> EventRecord:
    ...
    if system is None:
        system = SystemRecord(name=payload.system_id, environment=payload.environment, status="connected")
        session.add(system)
    else:
    ...
    dispatch_phone_call(session, record, escalation)
...
def ingest_event(payload: EventIngestion, principal: Principal = Depends(authenticate)) -> dict:
    with session_scope() as session:
        return event_dict(create_event(session, payload, principal))
...
def list_events(
    ...
    with session_scope() as session:
        ...
        if severity:
            statement = statement.where(EventRecord.severity == normalize_severity(severity))
        if status:
...
def update_event(event_id: int, update: EventUpdate, principal: Principal = Depends(authenticate)) -> dict:
    with session_scope() as session:
        ...
        if event is None:
            raise HTTPException(status_code=404, detail="event not found")
        if principal.environment != event.environment and "*" not in principal.scopes:
            raise HTTPException(status_code=403, detail="client cannot update events in another environment")
        requested_status = update.status
        if update.resolve:
            requested_status = "resolved"
        elif update.acknowledge:
            requested_status = "acknowledged"
        if requested_status is None:
            raise HTTPException(status_code=422, detail="provide status, acknowledge or resolve")
