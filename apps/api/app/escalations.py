...
def process_overdue_escalations(session) -> list[dict]:
    ...
    for record in rows:
        ...
        event = session.get(EventRecord, record.event_id); old_priority = record.priority; new_priority = NEXT_PRIORITY.get(old_priority, old_priority)
        if new_priority != old_priority:
            record.priority = new_priority; record.action = ACTION_BY_PRIORITY[new_priority]
            if event: event.priority = new_priority; event.recommended_action = record.action; event.updated_at = now
            record.status = "dispatched"; record.created_at = now; record.updated_at = now; session.add(AuditRecord(type="escalation_timeout", reference_id=str(record.id), detail=f"event={record.event_id} {old_priority}->{new_priority}; redispatched after missed SLA")); changed.append(escalation_dict(record))
        elif old_priority == "P1":
            record.status = "dispatched"; record.created_at = now; record.updated_at = now; session.add(AuditRecord(type="escalation_timeout", reference_id=str(record.id), detail=f"event={record.event_id} P1 re-dispatched after missed SLA")); changed.append(escalation_dict(record))
    return changed
