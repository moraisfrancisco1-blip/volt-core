...
class AgentInvestigationRecord(Base):
    ...
    proposed_files: Mapped[Any | None] = mapped_column(JSON, nullable=True)
    ...
    sandbox_network_isolated: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
    sandbox_ran_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
...
class MarketIntelligenceReportRecord(Base):
    __tablename__ = "market_intelligence_reports"
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    status: Mapped[str] = mapped_column(String(32), default="completed", index=True)
    competitors_summary: Mapped[str | None] = mapped_column(Text, nullable=True)
    regulation_summary: Mapped[str | None] = mapped_column(Text, nullable=True)
    price_signals_summary: Mapped[str | None] = mapped_column(Text, nullable=True)
    industry_news_summary: Mapped[str | None] = mapped_column(Text, nullable=True)
    model: Mapped[str | None] = mapped_column(String(64), nullable=True)
    turns_used: Mapped[int | None] = mapped_column(Integer, nullable=True)
    input_tokens: Mapped[int | None] = mapped_column(Integer, nullable=True)
    output_tokens: Mapped[int | None] = mapped_column(Integer, nullable=True)
