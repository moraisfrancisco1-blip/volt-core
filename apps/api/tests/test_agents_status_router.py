from app.agents import agent_inbox, backoffice_agent, customer_agent, dai_oakes_intelligence, dai_oakes_marketing, deals_agent, market_intelligence, marketing_agent, operations_agent, production_monitor, sales_agent
...
def test_agents_status_returns_all_fourteen_agents_with_valid_states(monkeypatch):
    ...
    monkeypatch.setattr(dai_oakes_marketing, "_sweep_in_progress", False)
    ...
    monkeypatch.setattr(operations_agent, "_sweep_in_progress", False)
    monkeypatch.setattr(backoffice_agent, "_sweep_in_progress", False)
    monkeypatch.setattr(customer_agent, "_sweep_in_progress", False)
    ...
    assert set(results.keys()) == {"volt", "dev_debug", "database", "finance", "production_monitor", "market_intelligence", "dai_oakes_intelligence", "dai_oakes_marketing", "sales", "deals", "marketing", "operations", "backoffice", "customer"}
    for row in results.values():
        assert row["state"] in {"idle", "error", "working"}


def test_customer_sweep_in_progress_reports_working(monkeypatch):
    monkeypatch.setattr(agent_inbox, "_current_message_type", None)
    monkeypatch.setattr(production_monitor, "_sweep_in_progress", False)
    monkeypatch.setattr(customer_agent, "_sweep_in_progress", True)

    results = {row["agent"]: row for row in agents_status()}

    assert results["customer"]["state"] == "working"


def test_customer_failed_audit_reports_error_when_not_in_progress(monkeypatch):
    monkeypatch.setattr(agent_inbox, "_current_message_type", None)
    monkeypatch.setattr(production_monitor, "_sweep_in_progress", False)
    monkeypatch.setattr(customer_agent, "_sweep_in_progress", False)
    with session_scope() as session:
        session.add(AuditRecord(type="customer_sweep_failed", detail="boom"))

    results = {row["agent"]: row for row in agents_status()}

    assert results["customer"]["state"] == "error"
    assert results["customer"]["last_status"] == "failed"


def test_backoffice_sweep_in_progress_reports_working(monkeypatch):
    monkeypatch.setattr(agent_inbox, "_current_message_type", None)
    monkeypatch.setattr(production_monitor, "_sweep_in_progress", False)
    monkeypatch.setattr(backoffice_agent, "_sweep_in_progress", True)

    results = {row["agent"]: row for row in agents_status()}

    assert results["backoffice"]["state"] == "working"


def test_backoffice_failed_audit_reports_error_when_not_in_progress(monkeypatch):
    monkeypatch.setattr(agent_inbox, "_current_message_type", None)
    monkeypatch.setattr(production_monitor, "_sweep_in_progress", False)
    monkeypatch.setattr(backoffice_agent, "_sweep_in_progress", False)
    with session_scope() as session:
        session.add(AuditRecord(type="backoffice_sweep_failed", detail="boom"))

    results = {row["agent"]: row for row in agents_status()}

    assert results["backoffice"]["state"] == "error"
    assert results["backoffice"]["last_status"] == "failed"


def test_operations_sweep_in_progress_reports_working(monkeypatch):
    monkeypatch.setattr(agent_inbox, "_current_message_type", None)
    monkeypatch.setattr(production_monitor, "_sweep_in_progress", False)
    monkeypatch.setattr(operations_agent, "_sweep_in_progress", True)

    results = {row["agent"]: row for row in agents_status()}

    assert results["operations"]["state"] == "working"


def test_operations_failed_audit_reports_error_when_not_in_progress(monkeypatch):
    monkeypatch.setattr(agent_inbox, "_current_message_type", None)
    monkeypatch.setattr(production_monitor, "_sweep_in_progress", False)
    monkeypatch.setattr(operations_agent, "_sweep_in_progress", False)
    with session_scope() as session:
        session.add(AuditRecord(type="operations_sweep_failed", detail="boom"))

    results = {row["agent"]: row for row in agents_status()}

    assert results["operations"]["state"] == "error"
    assert results["operations"]["last_status"] == "failed"
...
def test_dai_oakes_marketing_sweep_in_progress_reports_working(monkeypatch):
    monkeypatch.setattr(agent_inbox, "_current_message_type", None)
    monkeypatch.setattr(production_monitor, "_sweep_in_progress", False)
    monkeypatch.setattr(dai_oakes_marketing, "_sweep_in_progress", True)

    results = {row["agent"]: row for row in agents_status()}

    assert results["dai_oakes_marketing"]["state"] == "working"


def test_dai_oakes_marketing_failed_audit_reports_error_when_not_in_progress(monkeypatch):
    monkeypatch.setattr(agent_inbox, "_current_message_type", None)
    monkeypatch.setattr(production_monitor, "_sweep_in_progress", False)
    monkeypatch.setattr(dai_oakes_marketing, "_sweep_in_progress", False)
    with session_scope() as session:
        session.add(AuditRecord(type="dai_oakes_marketing_content_failed", detail="boom"))

    results = {row["agent"]: row for row in agents_status()}

    assert results["dai_oakes_marketing"]["state"] == "error"
    assert results["dai_oakes_marketing"]["last_status"] == "failed"
